...
import com.example.clashofclans.Model.Building.Maker;
...
public class ThreadCannon implements Runnable{
    ...
    public ThreadCannon(ImageView imageView){
        maker = new Maker(imageView) ;
        War.buildings.add(maker);
        War.map.put(maker , imageView) ;
...
//                wait();
        ...
        War.buildings.remove(War.buildings.indexOf(maker));
        War.map.get(maker).setVisible(false);
        War.map.remove(maker) ;
    }
