...
import com.example.clashofclans.Model.Building.ElixirMaker;
import javafx.scene.image.ImageView;

import static java.lang.Thread.sleep;

public class ThreadElixirMaker implements Runnable{
    private ElixirMaker elixir;
    private ImageView imageView ;
    public ThreadElixirMaker(ImageView imageView){
        elixir = new ElixirMaker(imageView) ;
        War.buildings.add(elixir);
        War.map.put(elixir , imageView) ;
...
//                wait();
        ...
        War.buildings.remove(War.buildings.indexOf(elixir));
        War.map.get(elixir).setVisible(false);
        War.map.remove(elixir) ;
    }
    private boolean isAlive () {
        if(elixir.getHealth() <= 0 )
            return false ;
        else return true ;
    }
